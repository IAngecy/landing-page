...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    let isActive = containerRef === undefined;
    ...
    // Se containerRef foi fornecido, o cursor só funciona quando o mouse está dentro do container
    let containerEnterHandler: (() => void) | null = null;
    let containerLeaveHandler: (() => void) | null = null;
    let checkIfInside: ((e: MouseEvent) => void) | null = null;

    // Salvar referência do container para usar no cleanup
    const container = containerRef?.current;

    if (containerRef && container) {
      // Inicialmente esconder o cursor
      if (cursorRef.current) {
        cursorRef.current.style.display = "none";
      }

      containerEnterHandler = () => {
        isActive = true;
      ...
      containerLeaveHandler = () => {
        isActive = false;
        document.body.style.cursor = originalCursor;
        if (cursorRef.current) {
          cursorRef.current.style.display = "none";
        }
        // Limpar qualquer target ativo
        if (activeTarget) {
      ...
      // Verificar se o mouse já está dentro do container ao carregar
      checkIfInside = (e: MouseEvent) => {
        ...
        if (isInside && !isActive) {
          containerEnterHandler!();
        } else if (!isInside && isActive) {
          containerLeaveHandler!();
        }
      };

      window.addEventListener("mousemove", checkIfInside);
    } else {
      // Comportamento original: funciona em toda a página
      if (hideDefaultCursor) {
        document.body.style.cursor = "none";
      }
    }
    ...
    const enterHandler = (e: MouseEvent) => {
      // Se há container, só funciona quando ativo
      if (containerRef && !isActive) return;
    ...
    return () => {
      ...
      // Usar a referência salva do container no início do efeito
      if (container && containerRef) {
        if (containerEnterHandler) {
        ...
        if (checkIfInside) {
          window.removeEventListener("mousemove", checkIfInside);
        }
      }
